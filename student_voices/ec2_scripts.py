...
def get_coherence_script(config, setting, model_dir, config_path, results_path, log_file_name, region, cancel_fleet=True, script='', run_as_user='', delimiter='\n'):
    # To be run within an instance 
    ...
    command = 'python /home/ec2-user/efs/Student-Voices/student_voices/run_coherence.py -c '+str(config)+' -cp '+str(config_path)+' -md '+str(model_dir)+' -s '+str(setting)+' -rd '+str(results_path)
